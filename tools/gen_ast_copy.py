...
def define_visitor(name: str, to_visit: List[str]):
  ...
  source += f"\tstd::stack<T> result_{name.lower()};\n"
  source += f"}};\n\n"
...
def define_types(name: str, types: Dict[str, List[Member]]):
  ...
  source += f"\t\tT result;\n"
  source += f"\t\tif (!visitor.result_{name.lower()}.empty()) {{\n"
  source += f"\t\t\tresult = visitor.result_{name.lower()}.top();\n"
  source += f"\t\t\tvisitor.result_{name.lower()}.pop();\n"
  source += f"\t\t}}\n"
